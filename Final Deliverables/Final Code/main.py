...
def regpage():
    if request.method == 'POST':
        sub="Verify your email for AmberGlow"
        ...
        subject="Verify email for AmberGlow"
...
def loginpage():
    ...
    if request.method == 'POST':
        ...
        password = request.form['password']
        query = "select COUNT(*)from user where usename='"+uname+"' and password='"+password+"'"
        stmt5 = ibm_db.exec_immediate(conn,query)
        row = ibm_db.fetch_tuple(stmt5)
        query1="select * from user where usename='"+uname+"' and password='"+password+"'"
...
Thank you for your order!. Your order has been received. Continue Shopping with AmberGlow"""
    for item in arr:
